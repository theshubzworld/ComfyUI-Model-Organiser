...
import { 
  ...
  ShieldAlert, ShieldCheck, HardDrive, ArrowRight, Wrench, CheckCircle2, Filter
} from 'lucide-react';
...
export function LinkAnalyzerModal({ isOpen, onClose, modelsList, onUpdateModel, onBulkUpdateModels }) {
  ...
  const [activeFilter, setActiveFilter] = useState('all'); // 'all' | 'missing' | 'search' | 'name' | 'size' | 'fixable'
  ...
  const fixableIssues = issues.filter(i => (i.suggestedUrl || i.suggestedName) && !fixedIds.has(i.id));
  ...
  const filteredIssues = issues.filter(item => {
    ...
    if (activeFilter === 'fixable') return Boolean(item.suggestedUrl || item.suggestedName);
    if (activeFilter !== 'all') return item.type === activeFilter;
    return true;
  });

  const handleFixSingle = (modelId, newUrl, newSize, newName) => {
    const updates = {};
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Audits download URLs, missing sizes, and placeholder filenames across your catalog
              </p>
            </div>
          </div>
              ...
              {filteredIssues.length === 0 ? (
                <div className="py-12 flex flex-col items-center justify-center space-y-3 theme-surface rounded-2xl border p-6 text-center">
                  ...
                  <p className="text-xs text-slate-400 max-w-md">
                    All audited download links and filenames are healthy and resolved.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {filteredIssues.map((item, idx) => {
                    const hasFix = Boolean(item.suggestedUrl || item.suggestedName);
                    return (
                      <div key={item.id || idx} className="theme-surface p-4 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all hover:border-indigo-500/40">
                        ...
                        <div className="space-y-1.5 max-w-xl">
                          ...
                          <div className="text-xs font-mono text-slate-400 truncate max-w-md">
                            Current: <span className="text-slate-300">{item.currentUrl || 'None'}</span>
                          </div>

                          {hasFix && (
                            <div className="text-xs font-mono text-emerald-400 flex items-center gap-1.5 truncate max-w-md">
                              <ArrowRight className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                              <span>Suggested: {item.suggestedUrl || item.suggestedName}</span>
                              {item.suggestedSize && <span className="text-emerald-300 font-bold">({item.suggestedSize})</span>}
                            </div>
                          )}
                        </div>
                        ...
                        {hasFix ? (
                          <button
                            onClick={() => handleFixSingle(item.id, item.suggestedUrl, item.suggestedSize, item.suggestedName)}
                            className="h-8 px-3 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-white flex items-center gap-1.5 shrink-0 shadow-md shadow-emerald-600/20 active:scale-95 transition-all"
                          >
